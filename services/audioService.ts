...
export type SoundType = 'CLICK' | 'SUCCESS' | 'ERROR' | 'LEVEL_UP' | 'GAME_OVER' | 'TYPE';
...
const SCALES = {
  ...
  COMBAT: [110.00, 116.54, 130.81, 146.83, 164.81, 174.61, 196.00, 220.00],
  // Whole Tone (Dreamy/Magical)
  MAGICAL: [261.63, 293.66, 329.63, 369.99, 415.30, 466.16, 523.25]
};
  ...
  const scheduleNotes = () => {
    ...
    while (nextNoteTime < now + 0.5) {
      ...
      if (mood === 'peaceful' || mood === 'fantasy' as any) {
        ...
        if (Math.random() > 0.3) {
            playLuteNote(scale[Math.floor(Math.random() * scale.length)], nextNoteTime, 0.3);
        }
        ...
        playLuteNote(scale[Math.floor(Math.random() * scale.length)], nextNoteTime, 3.0);
        // Add a detuned interval
        playLuteNote(scale[Math.floor(Math.random() * scale.length)] + 5, nextNoteTime, 3.0);
        ...
        nextNoteTime += speed;
         ...
         if (beatCount % 3 === 0) playLuteNote(scale[(idx + 2) % scale.length], nextNoteTime, 1.5);
        ...
        const scale = SCALES.DORIAN;
        playLuteNote(scale[Math.floor(Math.random() * scale.length)], nextNoteTime, 1.5);
        nextNoteTime += 0.8;
      }
...
export const speakText = (text: string, settings?: AudioSettings, overrideRate?: number) => {
  ...
  if (!selectedVoice) {
    selectedVoice = voices.find(v => 
      (v.name.includes("UK English Male") || v.name.includes("Google UK English Male"))
    );
    if (!selectedVoice) {
  ...
  // Rate & Pitch Logic
  // If overrideRate is provided (e.g., clicking "Slow" button), use it directly.
  // Otherwise use the settings multiplier.
  // Default base for medieval tone is 0.85 rate, 0.8 pitch.
  ...
  if (overrideRate) {
    ...
    // Combine base medieval style with user preference
    // If user chose a specific voice that isn't the default, they might expect normal speed (1.0) * userRate
    // But to keep consistency, we'll just multiply.
